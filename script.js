...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (pushBtn) {
    pushBtn.addEventListener("click", () => {
      console.log("Push gestartet");
      startTraining('push');
    });
  } else {
    console.warn("Push-Button nicht gefunden");
  }

  if (pullBtn) {
    pullBtn.addEventListener("click", () => {
      console.log("Pull gestartet");
      startTraining('pull');
    });
  } else {
    console.warn("Pull-Button nicht gefunden");
  }
});
...
function startTraining(type) {
  currentTraining = { type, date: new Date().toLocaleString(), exercises: [] };
  currentIndex = 0;
  document.getElementById("exerciseContainer").style.display = "block";
  document.querySelectorAll('main > *:not(#exerciseContainer)').forEach(e => e.style.display = 'none');
  renderExercise();
  currentType = type;
  currentIndex = 0;
  currentEntries = [];
